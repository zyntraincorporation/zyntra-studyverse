...
function getNextInterval(currentInterval, result, confidence) {
  ...
  if (result === 'correct' && confidence >= 4) return INTERVALS[Math.min(base + 1, INTERVALS.length - 1)];
  if (result === 'correct' && confidence >= 2) return INTERVALS[base];
  return INTERVALS[0];
}
...
async function recordReview(userId, { wordId, mode, result, confidence, responseMs }) {
